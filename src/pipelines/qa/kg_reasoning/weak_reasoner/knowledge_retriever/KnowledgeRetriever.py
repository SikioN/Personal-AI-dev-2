...
from .configs import KR_MAIN_LOG_PATH, AVAILABLE_QUADRUPLETS_FILTERS, AVAILABLE_QUADRUPLETS_RETRIEVERS
from .utils import BaseGraphSearchConfig, BaseQuadrupletsFilterConfig
...
from ......utils.data_structs import QueryInfo, Quadruplet, create_id
...
class KnowledgeRetrieverConfig:
    ...
    :param filter_config: Значение по умолчанию QuadrupletsFilterConfig().
    :type filter_config: BaseQuadrupletsFilterConfig
    ...
    filter_config: Union[BaseQuadrupletsFilterConfig, Dict] = field(default_factory=lambda: QuadrupletsFilterConfig())
...
class KnowledgeRetriever(CacheUtils):
    ...
    def __init__(self, kg_model: KnowledgeGraphModel, config: KnowledgeRetrieverConfig = KnowledgeRetrieverConfig(),
        ...
        self.graph_retriever = AVAILABLE_QUADRUPLETS_RETRIEVERS[self.config.retriever_method]['class'](
        ...
        if self.config.filter_method is None:
            self.quadruplets_filter = None
        else:
            self.quadruplets_filter = AVAILABLE_QUADRUPLETS_FILTERS[self.config.filter_method]['class'](
    ...
    def get_cache_key(self, query_info: QueryInfo) -> List[object]:
        return [self.config.retriever_method, self.graph_retriever.config.to_str(), str(self.config.filter_method), 
                self.quadruplets_filter.config.to_str() if self.quadruplets_filter is not None else "None", query_info.to_str()]

    @CacheUtils.cache_method_output
    def retrieve(self, query_info: QueryInfo) -> Tuple[List[Quadruplet], ReturnInfo]:
        """Метод предназначен для извлечения релевантных к user-вопросу квадруплетов из графа знаний.

        :param query_info: Структура данных, которая хранит user-вопрос и связанную с ним информацию.
        :type query_info: QueryInfo
        :return: Кортеж из двух объектов: (1) список релевантных user-вопросу квадруплетов; (2) статус завершения операции с пояснительной информацией.
        ...
        info = ReturnInfo()
        self.log("STAGE #3.1 - QUADRUPLETS EXTRACTION...", verbose=self.config.verbose)
        quadruplets = self.graph_retriever.get_relevant_quadruplets(query_info)
        self.log(f"RESULT: {len(quadruplets)}", verbose=self.config.verbose)
        for quadruplet in quadruplets:
            self.log(f"*[{quadruplet.id}] {quadruplet}", verbose=self.config.verbose)

        self.log("STAGE #3.2 - QUADRUPLETS FILTERING...", verbose=self.config.verbose)
        if self.quadruplets_filter is not None:
            filtered_quadruplets = self.quadruplets_filter.apply_filter(query_info, quadruplets)
            self.log(f"RESULT: {len(filtered_quadruplets)}", verbose=self.config.verbose)
            for quadruplet in filtered_quadruplets:
                self.log(f"*[{quadruplet.id}] {quadruplet}", verbose=self.config.verbose)
        else:
            filtered_quadruplets = quadruplets
            self.log("Stage was omited!", verbose=self.config.verbose)

        if len(filtered_quadruplets) == 0:
        ...
        return filtered_quadruplets, info
